...
Код возврата каждой команды представлен enum структурой `ExitCode` имеющей всего два значения: `Success` и `Failure`.
Метод `run` возвращает список кодов возврата (по одному для каждой из команд конвеера). 
В текущей реализации при любой ошибке исполнения хотя бы в одной команде
пользователь увидит сообщение `Error: Execution failed`.
